...
def test_make_doc_id_slugifies_korean_filenames():
    assert make_doc_id("2025+교육활동+보호+매뉴얼(배포용).pdf") == "2025-교육활동-보호-매뉴얼-배포용"
...
def test_split_assigns_sequential_chunk_ids_per_page():
    doc = Document(page_content=("문단 " * 150) + "\n\n" + ("둘째 " * 150), metadata={"doc_id": "x", "page": 3})
    chunks = split_documents([doc], chunk_size=300, chunk_overlap=50)
    assert len(chunks) > 1
    assert [c.metadata["chunk_id"] for c in chunks] == [f"x:p3:c{i}" for i in range(len(chunks))]
    assert all(len(c.page_content) <= 300 for c in chunks)


def test_split_preserves_source_metadata_and_overlap():
    doc = Document(page_content="가나다라 " * 200, metadata={"doc_id": "x", "page": 1, "title": "T"})
